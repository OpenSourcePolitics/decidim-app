...
module Decidim
  class SmsGatewayService
    ...
    def sms_message
      return code if code.to_s.length > Decidim::EphemeralAuthorizationHandler.auth_code_length

      platform = fetch_configuration(:platform, required: false).presence || @organization_name
      I18n.t("decidim.sms_gateway_service.message", code: code, platform: platform)
    end

    def fetch_configuration(key, required: true)
      value = Decidim.config.sms_gateway_context&.[](key.to_sym)
      if required && value.blank?
        Rails.logger.error "Decidim::SmsGatewayService is missing a configuration value for :#{key}, " \
                           "please check Decidim.config.sms_gateway_context[:#{key}] in config/initializers/decidim.rb " \
