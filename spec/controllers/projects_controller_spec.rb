  ...
  module Budgets
    describe ProjectsController do
      ...
      describe "GET index" do
        ...
        context "with geocoded and non geocoded projects" do
        ...
        context "with voting finished, and selected and non selected projects" do
          let!(:component) { create(:budgets_component, :with_voting_finished) }
          let!(:selected) { create_list(:project, 3, :selected, budget:) }
          let!(:not_selected) { create_list(:project, 7, budget:) }

          it "returns all projects, selected and non selected" do
            get :index, params: { budget_id: budget.id }
            expect(response).to have_http_status(:ok)
            expect(subject).to render_template(:index)

            expect(subject.send(:projects).count).to eq 10
          end
        end
      end
    end
