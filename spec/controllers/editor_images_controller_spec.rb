    ...
    describe "POST create" do
      ...
      context "when admin is signed in" do
        ...
        it "returns full image url" do
          ...
          active_storage_path = Decidim::EditorImage.first.attached_uploader(:file).url(host: organization.host)
          expect(response.body).to eq({ url: active_storage_path, message: "Image uploaded successfully" }.to_json)
        end
        ...
        context "when development mode" do
          ...
          it "returns full image url" do
            ...
            expect(response.body).to eq({ url: "http://test.host:3000#{active_storage_path}", message: "Image uploaded successfully" }.to_json)
          end
        end
      end
    end
