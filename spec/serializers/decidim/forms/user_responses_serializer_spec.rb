      ...
      describe "#serialize" do
        ...
        context "and includes the attributes" do
          ...
          it "includes email" do
            key = I18n.t(:email, scope: "decidim.forms.user_responses_serializer")
            expect(serialized[key]).to eq user.email
          end

          it "includes name" do
            key = I18n.t(:name, scope: "decidim.forms.user_responses_serializer")
        ...
        context "when the questionnaire body is very long" do
          ...
          it "does not load the questionnaire description to memory every time when iterating an response" do
            ...
            # A memory leak would explode over 100_000 KB increase in this test, so we can be safe with this threshold
            # We adjust it to avoid false crashes in CI environments that could be a bit more noisy
            expect(memory_usage - initial_memory).to be < 20_000
          end
