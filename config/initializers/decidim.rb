...
Decidim.configure do |config|
  ...
  # config.available_locales = Decidim::Env.new("DECIDIM_AVAILABLE_LOCALES", "en,fr").to_array
  # Or block set it up manually and prevent ENV manipulation:
  config.available_locales = if Rails.env.test?
                               %w(en fr ca es)
                             else
                               Decidim::Env.new("DECIDIM_AVAILABLE_LOCALES", "en,fr").to_array
                             end

  # Sets the default locale for new organizations. When creating a new
  # organization from the System area, system admins will be able to overwrite
  # this value for that specific organization.
  config.default_locale = if Rails.env.test?
                            :en
                          else
                            Decidim::Env.new("DECIDIM_DEFAULT_LOCALE", "fr").to_s.to_sym
                          end

  # Restrict access to the system part with an authorized ip list.
  # You can use a single ip like ("1.2.3.4"), or an ip subnet like ("1.2.3.4/24")
  # You may specify multiple ip in an array ["1.2.3.4", "1.2.3.4/24"]
  system_ips = Decidim::Env.new("DECIDIM_SYSTEM_ACCESSLIST_IPS").to_array
  config.system_accesslist_ips = system_ips if system_ips.present?
  ...
  force_ssl_value = Decidim::Env.new("DECIDIM_FORCE_SSL", "auto").default_or_present_if_exists
  config.force_ssl = force_ssl_value.present? unless force_ssl_value.to_s == "auto"
  ...
  config.service_worker_enabled = Decidim::Env.new("DECIDIM_SERVICE_WORKER_ENABLED", Rails.env.exclude?("development")).to_boolean_string == "true"
  ...
  #   cache_prefix: "..."
  # }
  maps_provider = Decidim::Env.new("MAPS_PROVIDER", "here").to_s
  if maps_provider.present?
    maps_api_key = ENV["MAPS_API_KEY"] || ""
    dynamic_provider = Decidim::Env.new("MAPS_DYNAMIC_PROVIDER", maps_provider).to_s
    static_provider = Decidim::Env.new("MAPS_STATIC_PROVIDER", maps_provider).to_s
    geocoding_provider = Decidim::Env.new("MAPS_GEOCODING_PROVIDER", maps_provider).to_s

    config.maps = {
      provider: maps_provider.to_sym,
      ...
      dynamic: {
        provider: dynamic_provider.to_sym,
        api_key: Decidim::Env.new("MAPS_DYNAMIC_API_KEY", maps_api_key).to_s,
        tile_layer: {
          url: ENV["MAPS_DYNAMIC_URL"] || "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",
          attribution: ENV["MAPS_ATTRIBUTION"] || '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
        }
      },

      geocoding: {
        provider: geocoding_provider.to_sym,
        api_key: Decidim::Env.new("MAPS_GEOCODING_API_KEY", maps_api_key).to_s
      },

      static: {
        provider: static_provider.to_sym,
        url: ENV["MAPS_STATIC_URL"] || "https://image.maps.hereapi.com/mia/v3/base/mc/overlay"
      }
    }
  ...
  config.geocoder = {
    timeout: Decidim::Env.new("GEOCODER_TIMEOUT", 5).to_i,
    units: Decidim::Env.new("GEOCODER_UNITS", "km").to_s.to_sym
  }

  # Custom resource reference generator method.
  # Check Decidim::ResourceReferenceGenerator.
  ...
  # The maximum file size of an attachment. This limit is used for all attachments.
  config.maximum_attachment_size = Decidim::Env.new("DECIDIM_MAXIMUM_ATTACHMENT_SIZE", 10).to_i.megabytes

  # The maximum file size for user avatar uploads. This limit is used for user profile pictures.
  ...
  # environment and usage scenarios. This feature also comes with the risk that
  # an organization's administrator injects malicious scripts to spy on or
  # take over user accounts.
  #
  config.enable_html_header_snippets = Decidim::Env.new("DECIDIM_ENABLE_HTML_HEADER_SNIPPETS", true).to_boolean_string == "true"

  # Allow organizations admins to track newsletter links.
  # if this var is not defined, it is decided automatically per-rails-environment
  track_newsletter = Decidim::Env.new("DECIDIM_TRACK_NEWSLETTER_LINKS", "auto").default_or_present_if_exists
  config.track_newsletter_links = track_newsletter.present? unless track_newsletter.to_s == "auto"

  # Amount of time that the data portability export will be available for the user to download.
  config.download_your_data_expiry_time = Decidim::Env.new("DECIDIM_DOWNLOAD_YOUR_DATA_EXPIRY_TIME", 7).to_i.days

  # Max requests per IP in a throttling time window
  config.throttling_max_requests = Decidim::Env.new("DECIDIM_THROTTLING_MAX_REQUESTS", 100).to_i

  # Throttling time window in minutes
  config.throttling_period = Decidim::Env.new("DECIDIM_THROTTLING_PERIOD", 1).to_i.minute

  # Time that a user can access the application before being asked for their password again.
  config.unconfirmed_access_for = Decidim::Env.new("DECIDIM_UNCONFIRMED_ACCESS_FOR", 0).to_i.days

  # A base path for the uploads. If set, make sure it ends in a slash.
  # Uploads will be set to `<base_path>/uploads/`. This can be useful if you
  # want to use the same uploads folder for multiple environments.
  base_path = Decidim::Env.new("DECIDIM_BASE_UPLOADS_PATH").to_s
  config.base_uploads_path = base_path if base_path.present?
  ...
  # Decidim docs at https://docs.decidim.org/en/develop/services/etherpad.html
  # in order to set it up.
  #
  etherpad_server = ENV.fetch("ETHERPAD_SERVER", nil)
  etherpad_api_key = ENV.fetch("ETHERPAD_API_KEY", nil)
  if etherpad_server.present? && etherpad_api_key.present?
    config.etherpad = {
  ...
  # set cookies.
  consent_cookie = Decidim::Env.new("DECIDIM_CONSENT_COOKIE_NAME", "decidim-consent").to_s
  config.consent_cookie_name = consent_cookie if consent_cookie.present?
  ...
  cache_separator = Decidim::Env.new("DECIDIM_CACHE_KEY_SEPARATOR", "/").to_s
  config.cache_key_separator = cache_separator if cache_separator.present?

  expire_session = Decidim::Env.new("DECIDIM_EXPIRE_SESSION_AFTER", 30).to_i
  config.expire_session_after = expire_session.minutes if expire_session.positive?

  remember_me = Decidim::Env.new("DECIDIM_ENABLE_REMEMBER_ME", "auto").default_or_present_if_exists
  config.enable_remember_me = remember_me.present? unless remember_me.to_s == "auto"

  session_timeout = Decidim::Env.new("DECIDIM_SESSION_TIMEOUT_INTERVAL", 10).to_i
  config.session_timeout_interval = session_timeout.seconds if session_timeout.positive?

  config.follow_http_x_forwarded_host = Decidim::Env.new("DECIDIM_FOLLOW_HTTP_X_FORWARDED_HOST", false).to_boolean_string == "true"
  config.maximum_conversation_message_length = Decidim::Env.new("DECIDIM_MAXIMUM_CONVERSATION_MESSAGE_LENGTH", 1000).to_i

  password_similarity = Decidim::Env.new("DECIDIM_PASSWORD_SIMILARITY_LENGTH", 4).to_i
  config.password_similarity_length = password_similarity if password_similarity.positive?

  denied_passwords = Decidim::Env.new("DECIDIM_DENIED_PASSWORDS").to_array(separator: ", ")
  config.denied_passwords = denied_passwords if denied_passwords.present?

  allow_redirects = Decidim::Env.new("DECIDIM_ALLOW_OPEN_REDIRECTS", false).to_boolean_string
  config.allow_open_redirects = allow_redirects == "true"
end
...
if Decidim.module_installed? :meetings
  Decidim::Meetings.configure do |config|
    ...
    enable_linking = Decidim::Env.new("MEETINGS_ENABLE_PROPOSAL_LINKING", "auto").default_or_present_if_exists
    config.enable_proposal_linking = enable_linking.present? unless enable_linking.to_s == "auto"
  end
end

if Decidim.module_installed? :budgets
  Decidim::Budgets.configure do |config|
    enable_linking = Decidim::Env.new("BUDGETS_ENABLE_PROPOSAL_LINKING", "auto").default_or_present_if_exists
    config.enable_proposal_linking = enable_linking.present? unless enable_linking.to_s == "auto"
  end
end
